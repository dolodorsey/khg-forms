...
const SB = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://dzlmtvodpyhetvektfuo.supabase.co';
const SK = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6ImR6bG10dm9kcHloZXR2ZWt0ZnVvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk1ODQ4NjQsImV4cCI6MjA4NTE2MDg2NH0.qmnWB4aWdb7U8Iod9Hv8PQAOJO3AG0vYEGnPS--kfAo';

const FORM_META = {
  rsvp: {
    title: 'Free RSVP',
    subtitle: 'Request complimentary admission before the stated arrival deadline. RSVP does not guarantee entry after the deadline or when capacity is reached.',
    submit: 'Request free RSVP',
  },
  birthday: {
    title: 'Birthday Celebration',
    subtitle: 'Tell the team how you want to celebrate so the correct birthday package and arrival instructions can be assigned.',
    submit: 'Submit birthday request',
  },
  vendor: {
    title: 'Vendor Application',
    subtitle: 'Apply to sell food, products, art or services. Approval, placement and fees are confirmed separately.',
    submit: 'Submit vendor application',
  },
  table: {
    title: 'Table Request',
    subtitle: 'Request a table or section. A host will confirm availability, minimums, deposit and arrival time.',
    submit: 'Request a table',
  },
  sponsor: {
    title: 'Sponsorship Inquiry',
    subtitle: 'Share the audience, activation and commercial outcome your brand wants to achieve.',
    submit: 'Submit sponsorship inquiry',
  },
  media: {
    title: 'Media & Press Request',
    subtitle: 'Request credentials, interviews, photography access or official event coverage.',
...
export default function EventIntakeForm({ brandName, brandKey, tableName, accent = '#D4A853', formType }) {
  ...
  const requirementsMet = useMemo(() => {
    const base =
      common.first_name.trim().length > 0 &&
      common.last_name.trim().length > 0 &&
      /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(common.email) &&
      common.phone.replace(/\D/g, '').length >= 10 &&
      common.event_date;

    if (!base || state === 'submitting') return false;
    ...
    if (formType === 'sponsor') return common.company_name.trim() && details.budget_range && details.sponsorship_goals.trim();
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${SB}/rest/v1/${tableName}`, {
        method: 'POST',
        headers: {
          apikey: SK,
      ...
      if (!response.ok) {
        ...
        throw new Error(error?.message || 'Submission failed. Please review the form and try again.');
      }

      setState('success');
      setMessage(`${meta.title} received. The ${brandName} team will follow up using the contact information submitted.`);
  ...
  if (state === 'success') {
    return (
      <main className="page" style={{ '--accent': accent }}>
        <section className="shell success">
    ...
    <main className="page" style={{ '--accent': accent }}>
      <section className="shell">
        ...
        <form onSubmit={submit} noValidate>
          ...
          {formType === 'rsvp' && (
            <section className="form-section">
              ...
              <label className="check-row">
                <input type="checkbox" checked={details.arrival_agreement} onChange={(e) => setDetail('arrival_agreement', e.target.checked)} />
                <span>I understand free RSVP is subject to capacity and the arrival deadline shown in my confirmation.</span>
              </label>
            </section>
          )}
          ...
          {formType === 'birthday' && (
            <section className="form-section">
              ...
              <div className="grid two">
                ...
                <Field label="Flyer/photo link" optional><Input type="url" value={details.flyer_photo_url} onChange={(e) => setDetail('flyer_photo_url', e.target.value)} placeholder="Google Drive, Dropbox or social link" /></Field>
                <Field label="Preferred arrival" optional><Input value={details.preferred_arrival} onChange={(e) => setDetail('preferred_arrival', e.target.value)} placeholder="Example: 10:30 PM" /></Field>
              </div>
            </section>
          )}
          ...
          {formType === 'sponsor' && (
            <section className="form-section">
              ...
              <div className="grid two">
                ...
                <Field label="Industry"><Input value={details.industry} onChange={(e) => setDetail('industry', e.target.value)} /></Field>
                <Field label="Budget range"><Select value={details.budget_range} onChange={(e) => setDetail('budget_range', e.target.value)}><option value="">Select</option><option>Under $1,000</option><option>$1,000–$5,000</option><option>$5,000–$15,000</option><option>$15,000–$50,000</option><option>$50,000+</option><option>In-kind / product</option></Select></Field>
                <Field label="Decision timeline"><Select value={details.decision_timeline} onChange={(e) => setDetail('decision_timeline', e.target.value)}><option value="">Select</option><option>Immediately</option><option>Within 7 days</option><option>Within 30 days</option><option>Future event</option></Select></Field>
              </div>
          ...
          <label className="check-row sms">
            <input type="checkbox" checked={common.sms_consent} onChange={(e) => setCommonValue('sms_consent', e.target.checked)} />
  ...
  return (
    <style jsx global>{`
      * { box-sizing: border-box; }
      ...
      .page { min-height: 100vh; padding: 48px 20px 90px; background: radial-gradient(circle at 88% 0%, color-mix(in srgb, var(--accent) 22%, transparent), transparent 32%), #090909; color: #fff; font-family: 'DM Sans', Inter, sans-serif; }
      ...
      form, .success { padding: clamp(22px, 5vw, 42px); border: 1px solid rgba(255,255,255,.12); border-radius: 26px; background: rgba(18,18,18,.94); box-shadow: 0 30px 90px rgba(0,0,0,.45); }
      ...
      .success { margin-top: 12vh; text-align: center; }
      .success .check { display: grid; place-items: center; width: 68px; height: 68px; margin: 28px auto 18px; border-radius: 50%; background: color-mix(in srgb, var(--accent) 18%, transparent); color: var(--accent); font-size: 36px; }
      .success h1 { font-size: clamp(38px, 7vw, 64px); }
      .success p { max-width: 620px; margin: 0 auto; color: rgba(255,255,255,.68); line-height: 1.65; }
      @media (max-width: 700px) { .grid.two { grid-template-columns: 1fr; gap: 0; } .page { padding-top: 34px; } form, .success { border-radius: 20px; } }
    `}</style>
  );
}
